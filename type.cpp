 ...
 *		Since Simple C has only char, int, and long as specifiers,
 *		the actual specifier does not matter.
 */
 ...
 *
 * Description:	Check if this type is a logical type after any promotion.
 */
 ...
 *		numeric or are identical.
 */

bool Type::isCompatibleWith(const Type &that) const
{
    return (isNumeric() && that.isNumeric()) || (isLogical() && *this == that);
}
